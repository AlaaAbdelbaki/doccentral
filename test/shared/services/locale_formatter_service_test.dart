...
void main() {
  group('LocaleFormatterService — date', () {
    ...
    test('formats as DD/MM/YYYY for FR', () {
      expect(
        const LocaleFormatterService(Locale('fr')).formatDate(date),
        '31/05/2026',
      );
    });

    test('formats as YYYY-MM-DD for EN', () {
      expect(
        const LocaleFormatterService(Locale('en')).formatDate(date),
        '2026-05-31',
      );
    });
    ...
    test('formats as DD/MM/YYYY for AR', () {
      expect(
        const LocaleFormatterService(Locale('ar')).formatDate(date),
        '31/05/2026',
      );
    });
  });
  ...
  group('LocaleFormatterService — time', () {
    ...
    test('formats as HH:mm 24h for FR', () {
      expect(
        const LocaleFormatterService(Locale('fr')).formatTime(time),
        '14:30',
      );
    });
    ...
    test('formats as HH:mm 24h for EN', () {
      expect(
        const LocaleFormatterService(Locale('en')).formatTime(time),
        '14:30',
      );
    });
    ...
    test('formats as HH:mm 24h for AR', () {
      expect(
        const LocaleFormatterService(Locale('ar')).formatTime(time),
        '14:30',
      );
    });
  });
  ...
  group('LocaleFormatterService — currency', () {
    test('formats TND with 3 decimal places for FR', () {
      final result = const LocaleFormatterService(
        Locale('fr'),
    ...
    test('all supported locales produce non-empty date output', () {
      ...
      for (final locale in [
        ...
        expect(
          result,
          isNotEmpty,
          reason: '${locale.languageCode} date was empty',
        );
      }
    });
    ...
    test('all supported locales produce non-empty time output', () {
      ...
      for (final locale in [
        ...
        expect(
          result,
          isNotEmpty,
          reason: '${locale.languageCode} time was empty',
        );
      }
    });
  });
